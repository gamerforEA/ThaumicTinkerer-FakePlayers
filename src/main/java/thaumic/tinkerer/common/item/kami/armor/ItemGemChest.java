...
public class ItemGemChest extends ItemIchorclothArmorAdv
{
	...
	void tickPlayer(EntityPlayer player)
	{
		...
		if (!EventConfig.robeStratosphereProjectileProtect)
			return;
	...
	public IRegisterableResearch getResearchItem()
	{
		if (!ConfigHandler.enableKami)
			return null;
		return (IRegisterableResearch) new KamiResearchItem(LibResearch.KEY_ICHORCLOTH_CHEST_GEM, new AspectList().add(Aspect.AIR, 2).add(Aspect.MOTION, 1).add(Aspect.FLIGHT, 1).add(Aspect.ELDRITCH, 1), 17, 7, 5, new ItemStack(this)).setParents(LibResearch.KEY_ICHORCLOTH_ARMOR).setPages(new ResearchPage("0"), ResearchHelper.infusionPage(LibResearch.KEY_ICHORCLOTH_CHEST_GEM));
	...
	public void updatePlayerFlyStatus(LivingUpdateEvent event)
	{
		if (event.entityLiving instanceof EntityPlayer)
		{
			...
			ItemStack armor = player.getCurrentArmor(3 - this.armorType);
