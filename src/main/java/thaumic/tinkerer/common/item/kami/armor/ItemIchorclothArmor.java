...
public class ItemIchorclothArmor extends ItemArmor implements IVisDiscountGear, ISpecialArmor, ITTinkererItem
{
	...
	// static ArmorMaterial material = EnumHelper.addArmorMaterial("ICHOR", 0, new int[] { 3, 8, 6, 3 }, 20);
	static ArmorMaterial material = EnumHelper.addArmorMaterial("ICHOR", 0, new int[] { EventConfig.ichorArmor1, EventConfig.ichorArmor2, EventConfig.ichorArmor3, EventConfig.ichorArmor4 }, 20);
	...
	{
		list.add(StatCollector.translateToLocal("tc.visdiscount") + ": " + (this.armorType == 3 ? 3 : 4) + "%");
	}
	...
	{
		return new ArmorProperties(0, this.getArmorMaterial().getDamageReductionAmount(slot) * 0.0425, Integer.MAX_VALUE);
	}
	...
	public IRegisterableResearch getResearchItem()
	{
		if (!ConfigHandler.enableKami)
			return null;
		return this.armorType != 0 ? null : (IRegisterableResearch) new KamiResearchItem(LibResearch.KEY_ICHORCLOTH_ARMOR, new AspectList().add(Aspect.ARMOR, 2).add(Aspect.CLOTH, 1).add(Aspect.LIGHT, 1).add(Aspect.CRAFT, 1), 17, 5, 5, new ItemStack(this)).setWarp(4).setConcealed().setParents(LibResearch.KEY_ICHOR_CLOTH).setPages(new ResearchPage("0"), ResearchHelper.arcaneRecipePage(LibResearch.KEY_ICHORCLOTH_HELM), ResearchHelper.arcaneRecipePage(LibResearch.KEY_ICHORCLOTH_CHEST), ResearchHelper.arcaneRecipePage(LibResearch.KEY_ICHORCLOTH_LEGS), ResearchHelper.arcaneRecipePage(LibResearch.KEY_ICHORCLOTH_BOOTS));
