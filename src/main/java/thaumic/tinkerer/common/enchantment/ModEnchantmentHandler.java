...
public class ModEnchantmentHandler
{
	...
	public void onEntityDamaged(LivingHurtEvent event)
	{
		if (event.source.getEntity() instanceof EntityLivingBase)
		{
			...
			if (focusedStrikes > 0 || dispersedStrikes > 0 && !(event.entityLiving instanceof EntityPlayer))
			{
				...
				int lastTarget = heldItem.stackTagCompound.getInteger(this.NBTLastTarget);
				int successiveStrikes = heldItem.stackTagCompound.getInteger(this.NBTSuccessiveStrike);
				int entityId = event.entityLiving.getEntityId();

				if (lastTarget != entityId)
				{
					heldItem.stackTagCompound.setInteger(this.NBTSuccessiveStrike, 0);
					successiveStrikes = 0;
				}
				else
				{
					heldItem.stackTagCompound.setInteger(this.NBTSuccessiveStrike, successiveStrikes + 1);
					successiveStrikes = 1;
				}
	...
	public void onBreakBlock(BlockEvent.BreakEvent event)
	{
		...
		if (tunnel > 0)
		{
			...
			item.stackTagCompound.setFloat(this.NBTTunnelDirection, dir);
		}
	}
	...
	public void onGetHarvestSpeed(PlayerEvent.BreakSpeed event)
	{
		...
		if (tunnel > 0)
		{
			...
			if (heldItem.stackTagCompound != null && heldItem.stackTagCompound.hasKey(this.NBTTunnelDirection))
			{
				float oldDir = heldItem.stackTagCompound.getFloat(this.NBTTunnelDirection);
		...
		boolean desintegrateApplies = desintegrate > 0 && event.block.getBlockHardness(event.entityPlayer.worldObj, event.x, event.y, event.z) <= 1.5F;
