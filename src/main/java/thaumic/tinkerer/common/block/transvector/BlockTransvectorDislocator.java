...
public class BlockTransvectorDislocator extends BlockCamo
{
	...
	{
		this.icons[0] = IconHelper.forBlock(par1IconRegister, this, 0);
		this.icons[1] = IconHelper.forBlock(par1IconRegister, this, 1);
	}
	...
	public IRegisterableResearch getResearchItem()
	{
		...
		return (IRegisterableResearch) new TTResearchItem(LibResearch.KEY_DISLOCATOR, new AspectList().add(Aspect.TRAVEL, 2).add(Aspect.MECHANISM, 1).add(Aspect.ELDRITCH, 1), -6, 1, 3, new ItemStack(this)).setConcealed().setParents(LibResearch.KEY_INTERFACE).setParentsHidden("MIRROR").setPages(new ResearchPage("0"), ResearchHelper.arcaneRecipePage(LibResearch.KEY_DISLOCATOR)).setSecondary();
	}
	...
	public ThaumicTinkererRecipe getRecipeItem()
	{
		if (!Config.allowMirrors)
			return null;
		return new ThaumicTinkererArcaneRecipe(LibResearch.KEY_DISLOCATOR, LibResearch.KEY_DISLOCATOR, new ItemStack(this), new AspectList().add(Aspect.EARTH, 5).add(Aspect.ENTROPY, 5), " M ", " I ", " C ", 'M', new ItemStack(ConfigItems.itemResource, 1, 10), 'I', new ItemStack(ThaumicTinkerer.registry.getFirstBlockFromClass(BlockTransvectorInterface.class)), 'C', new ItemStack(Items.comparator));
	}
}
